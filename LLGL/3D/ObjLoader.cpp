...
std::optional<Object3D> ObjLoader::load()
{
    ...
    while(true)
    {
        ...
        else if(command == "vt")
        {
            ...
            if(!(m_in >> v)) { m_in.clear(); v = 0; }
            if(!(m_in >> w)) { m_in.clear(); w = 0; }
...
std::optional<Vertex> ObjLoader::read_vertex(std::istream& in)
{
    ...
    if(in.get() != '/')
        return Vertex{m_vertexes[vertex - 1], Colors::white};
    ...
    if(in.get() != '/')
        return Vertex{m_vertexes[vertex - 1], Colors::white, m_tex_coords[tex_coord - 1]};
