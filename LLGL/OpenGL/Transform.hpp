...
{
    OrthoArgs() = default;
    OrthoArgs(Rectd rect, double near_ = 0, double far_ = 1)
    : left(rect.left), right(rect.left + rect.width), bottom(rect.top + rect.height), top(rect.top), near(near_), far(far_) {}
...
struct PerspectiveArgs
{
    ...
    : fov(fov_), aspect(aspect_), near(near_), far(far_) {}
